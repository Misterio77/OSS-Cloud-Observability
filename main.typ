...
- *OSS and standardization can challenge proprietary lock-in*. Our data extraction step yielded some proprietary software (e.g. _CloudWatch_, _AzureWatch_), most of them specific to a single cloud vendor. These were, however, often accompanied by OSS tools, a reminder that some of the OSS tools are so ubiquitous that vendors are effectively forced to implement support for them, further showing the strength OSS has in standardization and interoperability. Practitioners should consider prioritizing OSS when adopting observability tooling that needs to interoperate, for example, in multi-cloud scenarios.
