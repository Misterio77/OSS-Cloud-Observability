...
each step. The reproduction package is made available in the supplementary material
...
exported into a CSV in the following format: \texttt{"Title",\allowbreak "Year",\allowbreak "DOI",\allowbreak "Link",\allowbreak "Abstract"}. This data is available in the reproduction package as \texttt{01\allowbreak -scopus\allowbreak -search\allowbreak -results\allowbreak .csv}.
...
\texttt{02-\allowbreak validation-\allowbreak set-\allowbreak script.\allowbreak py}. The authors then manually
...
\texttt{03-\allowbreak validation-\allowbreak set-\allowbreak results.\allowbreak json}.
...
\texttt{06-\allowbreak tool-\allowbreak selection-\allowbreak manual.\allowbreak yaml}.

\subsection{Tool relations}\label{tool-relations}

To find the relations between the tools, we downloaded their source code,
and, for each tool, programmatically matched occurrences of every other tools'
names in its codebase. This step can be reproduced using \texttt{07-\allowbreak
...
Table~\ref{table-selected-tools}, in the Appendix, contains the full set
...
list of tools with singular presence in studies, while a few tools such as
...
Section~\ref{tool-relations}, through matching Y's name on X's codebase,
